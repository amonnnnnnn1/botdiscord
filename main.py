...
def round_to_tens(number: int) -> int:
    # округление суммы до ближайших 10
...
def get_rate(amount: int) -> int:
    # возвращаем курс в зависимости от суммы
...
class ConvertModal(ui.Modal, title="Конвертация"):
    amount = ui.TextInput(label="Сумма (₽)", placeholder="Введите сумму")
...
class SellerConvertModal(ui.Modal, title="Конвертация для продавцов"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            embed.add_field(name="Курс ($)", value=f"{format_with_dots(rate)}$", inline=False)
            embed.add_field(name="Результат ($ | ʊ)", value=f"{format_with_dots(result)}$", inline=False)
            embed.add_field(name="Комиссия 5% + 5% ($)", value=f"{format_with_dots(commission_5)}$ + {format_with_dots(commission_5)}$", inline=False)
            embed.add_field(name="Итоговая комиссия (10%)", value=f"{format_with_dots(commission_10)}$", inline=False)
            embed.add_field(name="Присылать комиссию на счёт -> 142153\nФотку отправленной комиссии отправьте в закрытый тикет", value="\u200b", inline=False)
            ...
            if LOG_CHANNEL_ID != 0:
                ...
                if log_channel:
                    ...
                    log.add_field(name="Курс ($)", value=f"{format_with_dots(rate)}$", inline=True)
                    log.add_field(name="Результат ($ | ʊ)", value=f"{format_with_dots(result)}$", inline=True)
                    log.add_field(name="Комиссия 5% + 5% ($)", value=f"{format_with_dots(commission_5)}$ + {format_with_dots(commission_5)}$", inline=True)
...
class RatesView(ui.View):
    def __init__(self):
        ...
        self.add_item(SellerConvertButton())  # кнопка продавцов - серая
        self.add_item(AdditionalButton())     # кнопка дополнительно - серая
        self.add_item(ConvertButton())        # кнопка конвертировать - синяя
...
async def panelz(ctx):
    ...
    panel_text = (
        ...
        "От 10.000 ₽ - 4.500$\n\n"
        "Комиссия: 1% + 5%\n\n"
        "Выберите нужную кнопку ниже для конвертации."
    )
